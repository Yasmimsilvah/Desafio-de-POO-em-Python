...
class Batalha:
    ...
    def iniciar_combate(self):
        ...
        while self.combatente_a.esta_vivo() and self.combatente_b.esta_vivo():
            if turno % 2 != 0:
                atacante = self.combatente_a
                alvo = self.combatente_b
            else:
                atacante = self.combatente_b
                alvo = self.combatente_a

            if atacante.esta_vivo():
                print(f"\n--- TURNO {turno} - Ataca: {atacante.nome} ---")
        ...
        if self.combatente_a.esta_vivo():
            print(f"VITÓRIA! O vencedor é {self.combatente_a.nome}!")
            return self.combatente_a
        elif self.combatente_b.esta_vivo():
            print(f"VITÓRIA! O vencedor é {self.combatente_b.nome}!")
            return self.combatente_b
        else:
            print("EMPATE! Ambos os combatentes foram derrotados no mesmo turno!")
            return None

heroi = Personagem("Heroi Bravio", 100, 15)
vilao = Personagem("Goblin Fraco", 30, 8)

confronto = Batalha(heroi, vilao)
vencedor = confronto.iniciar_combate()