...
class Personagem:
    ...
    def usar_habilidade(self, indice, alvo):
        if 0 <= indice < len(self.habilidades):
            ...
            print(f"[{self.nome}] Habilidade de índice {indice} não encontrada.")
...
class AtaqueForte(Habilidade):
    def __init__(self):
        super().__init__("Ataque Forte", 0)

    def usar(self, atacante, alvo):
        dano = atacante.forca_base * 2
        print(f"[{atacante.nome}] executa um Ataque Forte contra [{alvo.nome}] causando {dano} de dano físico!")
        alvo.receber_dano(dano)

class BolaDeFogo(Habilidade):
    def __init__(self):
        super().__init__("Bola de Fogo", 10)

    def usar(self, atacante, alvo):
        dano = atacante.forca_base * 3 
        print(f"[{atacante.nome}] conjura uma Bola de Fogo contra [{alvo.nome}] causando {dano} de dano mágico!")
        alvo.receber_dano(dano)

guerreiro = Guerreiro("Conan", 100, 15, 10)
goblin = Monstro.criar_goblin_padrao("Goblin de Teste")

guerreiro.aprender_habilidade(AtaqueForte())
guerreiro.aprender_habilidade(BolaDeFogo())

print(f"\nHabilidades de {guerreiro.nome}: 0: {guerreiro.habilidades[0].nome}, 1: {guerreiro.habilidades[1].nome}")

guerreiro.usar_habilidade(0, goblin)

guerreiro.usar_habilidade(1, goblin)

print(f"O Goblin ainda está vivo? {goblin.esta_vivo()}")